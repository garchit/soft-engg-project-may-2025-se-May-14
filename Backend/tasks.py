...
def calculate_student_rank(student_data, all_students_data):
    def performance_score(s): return (s['completion_rate'] * 0.4 + s['avg_marks'] * 0.3 + s['engagement_score'] * 0.3)
    sorted_students = sorted(all_students_data, key=performance_score, reverse=True)
    student_id = student_data['student'].id
    for rank, student in enumerate(sorted_students, 1):
        if student['student'].id == student_id: return rank
    return len(all_students_data)

def get_student_badges_info(student_id):
    badges_data = (db.session.query(Badge.name, Badge.points, Badge.description).join(badge_user, badge_user.c.badge_id == Badge.id).filter(badge_user.c.user_id == student_id).all())
    return {'badge_count': len(badges_data), 'total_badge_points': sum(badge.points for badge in badges_data)}

def generate_teacher_student_summary(teacher_id):
    students = (db.session.query(User).join(UserTeacher, UserTeacher.user_id == User.id).filter(UserTeacher.teacher_id == teacher_id).all())
    if not students: return None, f"No students found for teacher ID {teacher_id}"
    ...
    for student in students:
        ...
        student_data.append({'student': student, 'completion_rate': completion_rate, 'avg_marks': avg_marks, 'badges_info': badge_info, 'engagement_score': engagement_score})
...
def create_attractive_teacher_email_html(teacher, summary_data, ai_summary, month_year):
    ...
    <body style="font-family: 'Poppins', sans-serif; margin: 0; padding: 20px; background-color: #f4f7f9;">
        
        <div style="max-width: 680px; margin: auto; background: rgba(255, 255, 255, 0.7); border-radius: 16px; border: 1px solid #e5e7eb; overflow: hidden; box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.1); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);">
            
            <div style="padding: 30px 25px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; text-align: center;">
                <h1 style="margin: 0; font-size: 2rem; font-weight: 600;">Monthly Student Report</h1>
                <p style="margin: 5px 0 0; font-size: 1rem; opacity: 0.9;">{month_year} Summary for {teacher.name}</p>
            </div>
            
            <div style="padding: 30px;">
                
                <div style="background: rgba(255, 255, 255, 0.5); padding: 20px; border-radius: 12px; margin-bottom: 25px; border-left: 4px solid #667eea;">
                    <h3 style="margin:0 0 10px 0; color: #1a202c; font-weight: 600;">💡 AI Insights</h3>
                    <p style="margin:0; line-height: 1.6; color: #4a5568;">{ai_summary}</p>
                </div>
                
                <div style="display: flex; gap: 20px; flex-wrap: wrap; text-align: center; justify-content: space-around; margin-bottom: 30px; padding: 20px; background: rgba(243, 244, 246, 0.7); border-radius: 12px;">
                    <div><div style="font-size: 28px; font-weight: 700; color: #111827;">{summary_data['total_students']}</div><div style="font-size: 12px; color: #6b7280; text-transform: uppercase; font-weight: 500;">Students</div></div>
                    <div><div style="font-size: 28px; font-weight: 700; color: #111827;">{summary_data['avg_completion']:.0f}%</div><div style="font-size: 12px; color: #6b7280; text-transform: uppercase; font-weight: 500;">Avg. Completion</div></div>
                    <div><div style="font-size: 28px; font-weight: 700; color: #111827;">{summary_data['avg_engagement']:.0f}</div><div style="font-size: 12px; color: #6b7280; text-transform: uppercase; font-weight: 500;">Avg. Engagement</div></div>
                </div>

                <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                    <div style="flex: 1; min-width: 200px;">
                        <h3 style="color: #166534; font-weight: 600;">🏆 Top Performers</h3><ul style="padding-left: 20px; margin: 0; color: #374151; list-style-type: none;">{top_performers_html}</ul>
                    </div>
                    <div style="flex: 1; min-width: 200px;">
                        <h3 style="color: #b45309; font-weight: 600;">⚡ Needs Support</h3><ul style="padding-left: 20px; margin: 0; color: #374151; list-style-type: none;">{needs_attention_html}</ul>
                    </div>
                </div>
            </div>
        </div>
...
def send_monthly_class_reports_to_teachers():
    ...
    except ValueError as e: print(f"Teacher reports: Gemini setup failed - {e}")
    
    with app.app_context():
        teachers = Teacher.query.filter(Teacher.email.isnot(None), Teacher.email != '').all()
        if not teachers: return {'status': 'success', 'message': 'No teachers found', 'processed': 0}
...
def create_parent_email_html(progress_data, ai_body):
    ...
    <body style="font-family: 'Poppins', sans-serif; margin: 0; padding: 20px; background-color: #f4f7f9;">
        ...
        <div style="max-width: 600px; margin: auto; background: rgba(255, 255, 255, 0.7); border-radius: 16px; border: 1px solid #e5e7eb; overflow: hidden; box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.1); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);">

            <div style="padding: 30px; text-align:center; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
                <h1 style="margin:0; font-size:2rem; font-weight: 600;">✨ Progress Report</h1>
                <p style="margin:5px 0 0; font-size: 1rem; opacity: 0.9;">For {student_name}</p>
            </div>
            ...
            <div style="display:flex; justify-content:space-around; text-align:center; padding: 25px 20px; background-color: rgba(243, 244, 246, 0.7); border-top: 1px solid #e5e7eb;">
                <div>
                    <div style="font-size:2rem; font-weight: 700; color:#111827;">#{progress_data['overall_rank']}</div>
                    <div style="font-size:0.8rem; color:#6b7280; text-transform: uppercase; font-weight: 500;">Overall Rank</div>
                </div>
                <div>
                    <div style="font-size:2rem; font-weight: 700; color:#111827;">{progress_data['average_score']}%</div>
                    <div style="font-size:0.8rem; color:#6b7280; text-transform: uppercase; font-weight: 500;">Average Score</div>
                </div>
                <div>
                    <div style="font-size:2rem; font-weight: 700; color:#111827;">🔥 {progress_data['streak']}</div>
                    <div style="font-size:0.8rem; color:#6b7280; text-transform: uppercase; font-weight: 500;">Day Streak</div>
                </div>
            </div>
        </div>
    </body>
...
def send_progress_report_to_parent(student_id):
    ...
    gemini_client = None
    try: gemini_client = setup_gemini_client()
    except ValueError as e: print(f"Parent reports: Gemini setup failed - {e}")
    with app.app_context():
